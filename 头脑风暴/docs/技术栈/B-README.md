...
凡是接口接收用户自由文本（标题、内容、消息、备注、反馈等）的字段，**必须在 schema 层声明为 `SafeText`**（位于 `app/core/text_sanitize.py`），统一启用纵深防御清洗：

- `SafeText = Annotated[str, BeforeValidator(sanitize_bet_text)]`
- `BeforeValidator` 保证「先清洗、再校验长度」：恶意输入（如 `<img onerror=...>`）被清洗为空字符串后会被 `min_length=1` 拦截，而不是放过去落库
- 清洗内容：HTML / XML 标签、C0 / C1 / DEL 控制字符、零宽与不可见字符（ZWSP/ZWJ/BOM/双向控制等）；保留 emoji 与中日韩标点
- 不负责：SQL 注入转义（ORM 参数化查询已兜底）、敏感词命中（业务层职责）、长度截断（`Field(max_length=...)` 负责）

使用示例：

```python
from app.core.text_sanitize import SafeText
from pydantic import BaseModel, Field

class CreateBetRequest(BaseModel):
    # 必填自由文本：用 SafeText
    title: SafeText = Field(..., min_length=1, max_length=120)
    # 可选自由文本：用 SafeText | None
    reconfirmMessage: SafeText | None = Field(default=None, min_length=1, max_length=300)
```

新增任何接收用户自由文本的接口时，字段类型必须用 `SafeText`（必填）或 `SafeText | None`（可选），**不要用裸 `str`**。前端在 `frontend/couple-diary-f/src/uni-module-common/utils/sanitize-text.ts` 有对应的 `sanitizeTextInput` 公共纯函数，清洗规则与后端完全一致，形成前后端双层防御。详细规则见 `frontend/couple-diary-f/.claude/rules/UniApp模块化工程规范.md` 第 3.7 节。
