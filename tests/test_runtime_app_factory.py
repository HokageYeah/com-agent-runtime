...
def _runtime_dependencies(
    factory: sessionmaker, *, environment: str = "test"
) -> RuntimeDependencies:
    harness = RuntimeHarnessConfig(
        factory,
        {"test": {"keys": {"test": "random"}}},
        "runtime-test",
        "http://127.0.0.1:8765",
    )
    return RuntimeDependencies(
        _runtime_settings(environment=environment),
        factory,
        None,
        None,
        None,
...
def test_create_runtime_app_accepts_only_runtime_dependencies() -> None:
    assert tuple(signature(create_runtime_app).parameters) == ("dependencies",)


def test_create_runtime_app_uses_explicit_dependencies() -> None:
    factory = sessionmaker(bind=create_engine("sqlite://"))
    dependencies = _runtime_dependencies(factory)
    ...
    assert app.state.settings is dependencies.settings
...
def test_create_runtime_app_uses_injected_environment_without_global_settings(
    ...
    runtime_app = create_runtime_app(
        _runtime_dependencies(factory, environment="production")
    )
