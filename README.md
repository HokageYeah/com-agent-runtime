...
> **2026-09-11 新增计划，2026-09-12 已实施并通过双轴只读评审：** Memoir 默认配乐与服务器开关，见[技术主定义](/Users/yuye/YeahWork/Python项目/couple-diary-doc/头脑风暴/docs/superpowers/回忆录/designs/2026-09-11-M8默认配乐与生成开关设计说明.md)和[开发提示词](/Users/yuye/YeahWork/Python项目/couple-diary-doc/头脑风暴/docs/superpowers/回忆录/plans/2026-09-11-M8默认配乐开发提示词.md)。仅 Runtime 新增两个设置（`MEMOIR_MUSIC_GENERATION_ENABLED` 默认 false、`MEMOIR_DEFAULT_BGM_OBJECT_KEY`），默认源交付作品副本，TTS 不受配乐来源影响。自动化证据：7 文件套件 189 passed / 1 skipped（PG DSN 既有设计）/ 1 历史环境泄漏红 + ruff 绿，默认模式零火山提交/查询/下载已用例实证；真实源上传、RAM/ACL、试听与部署仍需人工。

> **2026-09-10 当前文档状态：** M7 已完成部署与测试（用户确认），实际最新已注册包1.0.7。M8 语音/配乐 R6–R8 与两轮必要修复已完成代码实现与隔离测试（新包 `1.0.8` 十一节点图 + 音频协调服务 + 账本/预算/门禁 + Dockerfile/env 模板，见 [VERIFICATION M8 R8](VERIFICATION.md)）；2026-09-10 第二轮必要修复落地 R3 时限裁剪 / R4 lease 接管栅栏 / R5 维护真实发布探测（维护 CLI 语义更新见 [ENV_CONFIG M8 维护段](ENV_CONFIG.md)），并修复发布对账三处 keyword-only 存量缺陷，见[第二轮必要修复完成记录](/Users/yuye/YeahWork/Python项目/couple-diary-doc/头脑风暴/docs/superpowers/回忆录/verification/2026-09-10-M8第二轮必要修复完成记录.md)。同日第三轮 C1 补超时音频孤儿生命周期：execute 先 reap 过窗持键 active（不刷新 `updated_at`）再扫描，`keep_published` 按对象成员关系判定，独立三件套 78 passed / 1 skipped。同日兼容补缺：缺字段/非法 `audio_object_keys` 未知保留，网关字段级跳过合法 digest，受影响面 182 passed / 1 skipped，见[第三轮必要补缺完成记录](/Users/yuye/YeahWork/Python项目/couple-diary-doc/头脑风暴/docs/superpowers/回忆录/verification/2026-09-10-M8第三轮必要补缺完成记录.md) §3.1 与 [VERIFICATION 兼容补缺节](VERIFICATION.md)。真实供应商小样本、镜像构建与部署注册由运维阶段执行，`MEMOIR_AUDIO_ENABLED` 默认关闭，Business 资格开关未开前现网零影响。旧包 1.0.0–1.0.7 字节与 digest 不变，公共 Runtime 消息/调度不改。阅读 [M8 设计](头脑风暴/docs/AgentRuntime/plans/2026-09-07-Memoir语音与配乐设计说明.md)、[R6–R8 计划](头脑风暴/docs/AgentRuntime/backend/2026-09-07-Memoir语音与配乐开发计划.md) 与 [ENV_CONFIG M8](ENV_CONFIG.md)。
...
│   │       ├── 1.0.0/ ... 1.0.8/             # 不可变的版本化 AgentPackage（1.0.8=M8 语音与配乐，默认未注册）
...
- `1.0.8`：M8 语音与配乐版本（已实现，未注册）。十一节点 DAG——在 `safety_review` 与 `publish_document` 之间插入 `enqueue_audio_tasks`（Memoir 专属音频服务编排：旁白按句无损分段逐段 TTS + ffmpeg 解码重拼 + 私有 OSS 上传；每作品一首 60s BGM 提交/轮询/下载/转码/上传；费用走 `memoir_audio_jobs`/`memoir_audio_run_budgets` 独立账本与 Run 级原子封顶），发布文档升 `schema_version=2.0.0`（新增 `audio.narrations`/`audio.background_music`，空音频为空数组+null）。`max_run_seconds` 300→1200 只对本包生效，音频节点受 min(300s, Run 剩余−30s 发布预留) 约束；`MEMOIR_AUDIO_ENABLED` 默认关闭，关闭时发布 2.0.0 空音频文档。Tool/Snapshot 契约不变，wire 合同仍为 `1.1.0`（登记表已含 1.0.8）。`1.0.0`–`1.0.7` 字节与 digest 冻结不变。
...
**2026-09-08 更新（1.0.8 落地状态）**：`1.0.8` 包与音频链路已实现并通过隔离回归（见 [VERIFICATION M8 R8](VERIFICATION.md)），但**未注册、未部署**——注册由运维阶段执行（业务侧资格开关默认关闭，Business connectivity 在开关打开时要求 {1.0.7, 1.0.8} 双包可用）。`AGENT_PACKAGE_VERSION` 维持 1.0.7 不变；启用音频需按 [ENV_CONFIG M8](ENV_CONFIG.md) 完成全部必填项并将 `MEMOIR_AUDIO_ENABLED` 置 true，同时重建含 ffmpeg 的镜像。旧包 digest 冻结断言与 wire 登记表全覆盖回归已进 `tests/runtime_test_memoir_package_versions.py` / `tests/test_runtime_contract_compatibility.py`。
