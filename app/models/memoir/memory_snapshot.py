...
class MemorySnapshot(Base):
    ...
    __table_args__ = (
        ...
        # 快照版本和 schema major 均为正数，未来 major 由服务层显式拒绝旧写入。
        CheckConstraint("snapshot_version >= 1", name="ck_memory_snapshot_version_positive"),
        CheckConstraint("schema_major >= 1", name="ck_memory_snapshot_schema_major_positive"),
    )
    ...
    # 加密快照负载的领域 schema major；未知未来 major 不允许旧服务覆盖。
