...
- image：图片场景卡，body 写生成一张配图所需的具体画面描述（谁、在哪里、做什么、关键物件或动作、什么氛围），不超过 80 个字；系统会在发布前把 body 送图像模型生成图片，图片生成失败时该场景自动退回 summary 文本卡。
image 场景的额外要求：先保留一张位于首位的 cover 文字卡，再为整本回忆录规划一张独立的整体封面 image 场景；之后对每个具有可靠具体画面信息的故事场景分别规划一张独立 image 场景，确保一个播放场景对应一张图片，不得复用同一 image 场景或同一素材描述。全文档 image 场景最多 6 个，达到上限后按画面信息的具体程度、故事代表性和情感细节取舍；纯数字、纯统计、纯对话或缺少可靠画面信息的场景不要强行生成图片。所有 image 场景都必须引用真实 source_refs。每个 image 场景可附带可选字段 title_word，为这张图片的标题词，必须是 1 到 6 个汉字（如"那年海边"），无合适标题词时省略该字段。
