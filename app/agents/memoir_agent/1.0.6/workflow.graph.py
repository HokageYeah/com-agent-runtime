...
#
# 1.0.6 图结构（与 1.0.5 完全一致的十节点 DAG，零结构变更）：本版本只改
# 循环体的批次重试语义（由 runner 按 agent_version 门控实现，不落图结构）——
# ①批次候选游标：单批模型调用与解析全部通过才提交素材消费进度，瞬时失败
# 下一轮同批重试，不再跳批；②首末批在场硬校验：首批首张必须 cover、末批
# 末张必须 summary，缺失整批拒绝交由同批重试；③定向结构修复请求携带
# required_scene_type 受信任字段。历史沿革：1.0.5（M7 动态生成）把 1.0.2 的
# 单次裁剪链（extract_highlights / plan_chapters）改为 bounded_loop
# generate_scene_batches 分批驱动 generate_scene_batch，每批一次模型调用、
# 由模型按素材丰富度动态决定场景数量与组织方式；覆盖缺失由唯一一次 repair
# 模型节点 repair_coverage_gaps 补齐。
#
