...
#
# safe_to_rerun 语义（R2 分类恢复 + M7 循环重算）：memoir 的读取/内容/发布节点
# 都声明 True——R2 checkpoint 不存正文，resume 时 state 为空，必须从 load_snapshot
# 重新读 Snapshot、内容节点整链重算、publish_document 走 query-after-commit
# （logical_key）幂等。bounded_loop 首版铁律 safe_to_rerun=True：循环中途不写
# checkpoint，Worker 崩溃/接管/retry/resume 后从节点起点按冻结排序完整重算，
# 整节点重算而非续跑。enqueue_media_tasks 是 optional 后处理，显式 False：
# 已完成则 resume 跳过（不重复提交媒体任务），未完成（partial 失败）才执行。
WORKFLOW_NODES = [
    {
    ...
    },
    {
        ...
        # 修复后新增的场景同样在此收口，保证 safety_review 时 actions 与 scenes 对齐。
        "node_type": "deterministic",
    ...
    },
    {
        ...
        # 发布后 optional 后处理：对每个最终 Scene 尝试一张配图，单场景失败只把
        # 该场景降级为文本卡并记 media_degraded 安全计数，不阻塞发布、不改变
        # succeeded/partial 判定。显式 safe_to_rerun=False：已完成则 resume 跳过
        # （不重复提交媒体任务），未完成（partial 失败）才执行。
        "node_type": "deterministic",
        "next_nodes": [],
        "optional": True,
        "safe_to_rerun": False,
